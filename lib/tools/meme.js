...
function Meme() {
  this.create = (param, callback) => {
    ...
    if(!param.alt) {
        if(!checkMemes(param.image)) return callback(new Error("Not a valid meme type");
        ...
        snekfetch.get(x)
            .then(x => {
            callback(x.body.direct.masked + "?font=" + param.font);
            });
        } else {
        ...
        x += param.bottom.replace('?', '~q').replace(/"/gi, '\'\'').replace(/_/gi, '__').replace(/-/gi, '--').replace(/ /gi, '_');
        if(param.font) {
            callback(x + '.jpg?font=' + param.font + '&alt=' + param.alt);
        }
        callback(x + '.jpg?alt=' + param.alt);
        }
    };
}
